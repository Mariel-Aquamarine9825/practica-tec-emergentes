...
def obtener_por_id(id):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute("SELECT id, first_name, last_name, city, address, salary precio FROM professor WHERE id = %s", (id,))
    professors = cursor.fetchone()
    return professors
@app.route("/formulario_editar_prof/<int:id>")
def editar_prof(id):
    professors = obtener_por_id(id)
    return render_template("editar.html",professors=professors)


@app.route("/actualizar_professor", methods=["POST"])
def actualizar_professor():
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    id = request.form["id"]
    first_name = request.form["first_name"]
    last_name = request.form["last_name"]
    ...
    address = request.form["address"]
    salary = request.form["salary"]
    cursor.execute("UPDATE professor SET first_name = %s, last_name = %s, city = %s, address = %s, salary = %s WHERE id = %s", (first_name, last_name, city, address, salary, id))
    mysql.connection.commit()
    return redirect("/professors")
